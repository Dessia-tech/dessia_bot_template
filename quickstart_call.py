...
def start_text():
    ...
    print("=======================\n")
    print(
        "This script will generate for you the files needed to create a python package respecting DessiA guidelines."
    )
    print(
        "If you don't understand a question or have no preference just type enter to have the default choice "
        "selected."
    )
    print(
        "Most questions have to be answered with y=yes or n=no. Default value if nothing is answered "
...
def create_package_base(parent_folder):
    ...
    git_use = input(
        "Do you want to use git in this project (eventually through a service like Github, Gitlab, Gitea, "
        "Gogs)?: (Y/n): "
    )
...
def get_misc():
    ...
    requirements = input(
        f"Enter required packages, separated by a comma (default : {default_requirements})"
    )
...
def misc(project_path, package_name, auth_name, auth_mail, s_desc, reqs, py_ver):
    ...
    # README
    create_readme = input(
        "Do you want to create a README file? It will be used as long description. (Y/n): "
    )
    ...
    qs.create_readme(
        create_readme, project_path, package_name, auth_name, auth_mail, s_desc
    )
    ...
    requirements, python_version = qs.change_requirements(
        reqs, default_requirements, py_ver
    )
...
def main():
    ...
    start_text()
    project_path, pkg_name, module_path, module_name, setup_path = create_package_base(
        parent_folder
    )
    s_desc, auth_name, auth_mail, reqs, py_ver = get_misc()
    requirement, python_version, pkg_version, short_desc = misc(
        project_path, pkg_name, auth_name, auth_mail, s_desc, reqs, py_ver
    )
    qs.create_project(
        setup_path,
        project_path,
    ...
    print(f"Project generated to {project_path}")
