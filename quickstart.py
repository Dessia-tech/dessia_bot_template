...
print(
    "Most questions have to be answered with y=yes or n=no. Default value if nothing is answered is written in capital\n"
)
...
git_use = input(
    "Do you want to use git in this project (eventually through a service like Github, Gitlab, Gitea, Gogs)?: (Y/n): "
)
...
if git_use:
    ...
    while not git_detected:
        project_path = input(f"Where is the git repo folder? current folder: {parent_folder}: ")
    ...
    base_folder = input(f"Select parent folder in which the project will be generated (default: {parent_folder}): ")
    if not base_folder:
    ...
    if os.path.exists(project_path):
        confirm = input(f"the folder {project_path} already exists. Confirm to use existing folder (y/N): ")
...
shutil.copyfile("logo.png", os.path.join(assets_path, f"{package_name}.png"))
...
requirements = input(f"Enter required packages, separated by a coma (default : {default_requirements})")
